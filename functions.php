...
function getDiscofeedACURL($entityID, $discofeedArray){
	
	global $SProviders;
	global $discofeedFile;
	...
	foreach($SProviders[$entityID]['ACURL'] as $ACURL){
		...
		$discofeedArray[$entityID]['ACURL'] = $ACURL;
		...
		if (strrpos($ACURL, $shib)){
			...
			if (!empty($feed)){
				$discofeedArray[$entityID]['FEED'] = $feed;
				dumpDiscoFile($discofeedFile, $discofeedArray, 'discofeedArray');
				return $feed;
			}
		}
	}

	$discofeedArray[$entityID]['FEED'] = '';
	dumpDiscoFile($discofeedFile, $discofeedArray, 'discofeedArray');
	return '';
}
...
/*******************************************************************************/
// Returns the value in the discofeed array if exists fetch discofeed via cURL
function retrieveDiscoFeed($entityID){

	global $discofeedFile;
	
	require_once($discofeedFile);

	if (isset($discofeedArray[$entityID])){
		return $discofeedArray[$entityID]['FEED'];
	}
	else {
		return getDiscofeedACURL($entityID, $discofeedArray);
	}
}

/******************************************************************************/
// Dump variable to a file 
function dumpDiscoFile($dumpFile, $providers, $variableName){

 	if(($fp = fopen($dumpFile, 'w')) !== false){
 		fwrite($fp, "<?php\n\n");
 		fwrite($fp, "// This file was automatically generated by functions.php\n");
